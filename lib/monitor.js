...
function track(container, data = {}) {
  ...
  if (tailf) {
    Promise
      .try(() => {
        return (new Producer({ uri : tailf, meta : data }).stream();
      })
      .then((stream) => {
        container.once('die', () => {
        ...
        container.on('stdout', (chunk) => stream.write({ chunk, type : 'stdout' }));
        container.on('stderr', (chunk) => stream.write({ chunk, type : 'stderr' }));
      })
      .catch((err) => {
