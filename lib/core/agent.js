var Promise         = require('bluebird')
  ...
  , read_packages   = require('read-package-json')
  ;
...
function Agent(pool) {
  ...
  if (this.capacity === 0) {
    ...
    var mem_os = Math.max(config.get('kernal.mem'), mem_mb / 16);
  ...
  this.worker_package_json = {};

  var package_json_path = path.join(config.get('worker.dir'), 'package.json');
  read_packages(package_json_path, console.error, true, function (err, data) {
    if (err) {
      console.error('There was an error reading the worker page.json at:', package_json_path);
      return
    }

    this.worker_package_json = data;
  }.bind(this));
}
...
Agent.prototype.start_heartbeat = function() {
  ...
  function ping(socket) {
    socket.emit('ping', {
      ...
      , worker_package_json : me.worker_package_json
    });
  }
