...
Agent.prototype.initialize = function(cb) {
  ...
  Promise
    ...
    .then(function(){
      ...
      var count = config.get('worker.count');
      if (count === 0) {
        count = Math.floor(os.freemem() / 1024 / 1024 /  config.get('worker.mem'))
      }

      var p$workers = _.times(count, function(){
...
Agent.prototype.handle = function(data, cb) {
  retry(function(){
  ...
  .then(function(worker){
    var id  = data.task.id
      , req = this.createReqStream({ id : id })
      ;
    ...
    worker.handle(req, undefined, cb);
  })
  .catch(cb);
};
