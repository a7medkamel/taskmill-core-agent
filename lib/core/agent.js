...
Agent.prototype.listen = function() {
  ...
  console.log('connecting to:', config.get('relay.url'));

  this.socket = require('socket.io-client')(config.get('relay.url'));
  ...
  ss(this.socket).on('request', function(ss$req, ss$res, data){
    // var ss$req = ss.createStream();
    // ss(this.socket).emit('req', ss$req, { id : data.task.id });
    ...
    this.handle(ss$req, ss$res, function(){
