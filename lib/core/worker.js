...
Worker.prototype.add_req = function(req, res, next){
  var id = req.task.id;
...
Worker.prototype.handle = function(data, next) {
  ...
  Promise
    .promisify(this.socket.emit, this.socket)('task', data.task)
    .bind(this)
    .then(function(){
      var worker = this;
    ...
    .catch(function(err){
      ...
      //   '#system' : {
      //       type    : 'exception'
      //       // todo [akamel] this makes for a nested error; flatten/refactor
      //     , error   : error.map(err)
      //   }
      // };
