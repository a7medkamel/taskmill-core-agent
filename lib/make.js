...
class Maker{
  ...
  static dirname(remote) {
    ...
    // todo [akamel] use git module normalize
    return path.join('.disk', _.toLower(url_parsed.hostname), _.toLower(url_parsed.pathname));
  }
...
function make(container, remote, sha, options = {}) {
  let { blob, blob_type, filename, token, cache, bearer, tailf } = options
    , time            = process.hrtime()
    , single_use      = (cache === false) || !!blob || !!tailf
    , { key, hash }   = make_sdk.key(remote, sha, { single_use })
    ;

  winston.info('make', remote, sha, key, hash);
