...
function track(container, result, options = {}) {
  let { tailf } = options;
  ...
  let monitor = (stats) => {
    ...
    make_sdk
      .set(result, { ttl : 5 })
      .catch((err) => {
        winston.error('error extend live container', err, result);
      });
  };

  if (tailf) {
    // let stdout = (new Producer(tailf)).stream(_.extend({ type : 'stdout' }, result));
    // let stderr = (new Producer(tailf)).stream(_.extend({ type : 'stderr' }, result));
    //
    // container.on('stdout', (chunk) => stdout.write(chunk.toString('utf-8')));
    // container.on('stderr', (chunk) => stderr.write(chunk.toString('utf-8')));
    let stdio = (new Producer(tailf)).stream(result);

    container.on('stdout', (chunk) => stdio.write({ chunk, meta : { type : 'stdout' }}));
    container.on('stderr', (chunk) => stdio.write({ chunk, meta : { type : 'stderr' }}));
  }
  ...
  container.once('die', () => {
    make_sdk
      .del(result.hash)
      .catch((err) => {
        winston.error('error unset dead container', err, result);
      });
  });
...
function make(container, remote, sha, options = {}) {
    ...
    function go_make() {
      ...
      return Promise
              ...
              .tap((result) => {
                return track(container, result, options);
              })
              .catch((err) => {
                winston.error(err);
                throw err.message;
