...
function track(container, result, options = {}) {
  ...
  if (tailf) {
    let stream = (new Producer(tailf)).stream(result);

    container.on('stdout', (chunk) => stream.write(chunk.toString('utf-8')));
    container.on('stderr', (chunk) => stream.write(chunk.toString('utf-8')));
  }
