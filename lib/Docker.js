...
class Docker extends EventEmitter {//require('dockerode') {
  constructor() {
    super();
  ...
  clean() {
    return Promise
            ...
            .each((item) => {
              var container = this.dockerode.getContainer(item.Id);
  ...
  create(options) {
    let cfg = {
      ...
      'keep-alive'  : _.get(options, 'keep-alive'),
  ...
  static native() {
    __native = __native || new require('dockerode')();
