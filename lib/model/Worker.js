...
class Worker extends EventEmitter {
  ...
  handle(req, res, options = {}) {
    ...
    req.profiler.start('worker.handle')

    return req.__obj
            .acl()
            .then(() => this.acquire(req))
            .then((container) => {
              req.profiler.start('worker/blob');
              req.profiler.start('container.repository');

              let blob = Promise
                          .try(() => {
                            let repository = container.repository();

                            req.profiler.done('container.repository');
                            let content   = req.__obj.doc.blob;

                            if (content) {
                              let filename  = req.__obj.doc.filename
                          ...
                          .tap(() => {
                            req.profiler.done('worker/blob');
                          })
                          ;
              
              req.profiler.start('container.build');
              Promise
                .all([
                    blob
                ...
                .spread((blob, build) => {
                  req.profiler.done('container.build')
                  let { sha } = build;
                ...
                .tap(() => req.profiler.done('worker.handle'))
                .catch((err) => {
  ...
  static get(req_obj = {}, options = {}) {
    ...
    // todo [akamel] verify that we are protecting private repos that are already up
    let sha = branch; // we assume we already converted branch to sha in the gateway
  ...
  static handle(req, res) {
    ...
    return Worker
            ...
            .catch((err) => {
              req.profiler.done('Worker::handle');
              winston.error(err);
