    ...
    if (!this.__container) {
      let single_use    = !this.options['keep-alive']
        ...
        , checksum      = crypto
                            ...
                            .update(this.id)
                            .digest('hex')
        , key           = filenamifyUrl(this.id, { replacement : '_' }) + '_' + checksum
    ...
    return req.__obj
            ...
            .then((container) => {
              ...
              let b = container
                        .repository()
                        .then((repository) => {
                          // todo [akamel] we no longer calculate manual on the fly
                          if (req.__obj.doc.blob) {
                            return { content : req.__obj.doc.blob };
                          }

                          return repository
                                  .blob(req.__obj.doc.filename, {
                        ...
                        .then((blob) => {
                          // console.log(blob.manual);
                          // todo [akamel] maybe we should rename to content? arg.....
                          req.__obj.doc.blob = blob.content;
