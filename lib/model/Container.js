...
class Container extends EventEmitter {
    ...
    return this
            ...
            .tap((container) => {
              container.stats({ stream : true }, (err, stream) => {
                // todo [akamel] end stream when contains dies?
                stream.on('data', (buf) => {
                  // todo [akamel] this can fail if partial data str
  ...
  handle(req, res) {
    ...
    console.log('pending: +', _.keys(this.__reqs));
  ...
  sigterm() {
    ...
    if (this.count() === 0) {
      ...
      return Promise
              .fromCallback((cb) => {
                this.on('drain', () => {
                  console.log('on drain:', this.count());
                  if (this.count() === 0) {
