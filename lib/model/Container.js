...
class Container extends EventEmitter {
  ...
  await() {
    return this
            ...
            .tap(() => {
              let { protocol, hostname } = config.getUrlObject('agent');
              // todo [akamel] generate key somewhere else
              // todo [akamel] include agent key/group in key
              // if (this.register) {
              //   return client.setAsync(`${this.remote}#${this.sha}`, `${protocol}//${hostname}:${this.port}`)
              // }
            });
